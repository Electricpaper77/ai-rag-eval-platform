...
def run(name, cases):
    ...
    value={"run_id":name,"metrics":metrics,"cases":cases}; value["checksum"]=run_checksum(value); return value
...
def test_safety_regression_blocks_and_missing_metrics_are_na():
    base=run("b",[{"case_id":"s","risk_category":"prompt_injection","pass":True}]); candidate=run("c",[{"case_id":"s","risk_category":"prompt_injection","pass":False}]); candidate["metrics"].pop("citation_precision"); candidate["checksum"]=run_checksum(candidate)
    result=compare(base,candidate)
