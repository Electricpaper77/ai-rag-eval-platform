...
def fixtures(s):
 ...
 if s=='escalate': c=run('candidate-ci',[{'case_id':'citation','risk_category':'citation','pass':False,'failure_reasons':['citation missing']},c['cases'][1]])
 return b,c
def checkout_runs():
 d=Path(tempfile.mkdtemp());out=[]
 for name in ('baseline-canonical','candidate-current-checkout'):
...
def main(argv=None):
 a=argparse.ArgumentParser();a.add_argument('--scenario',choices=('ship','block','escalate'));a.add_argument('--output',type=Path,default=Path('artifacts/ci_release_proof.json'));x=a.parse_args(argv)
 try:
  r=compare(*(fixtures(x.scenario) if x.scenario else checkout_runs()));validate(r);x.output.parent.mkdir(parents=True,exist_ok=True);x.output.write_text(json.dumps(r['proof'],indent=2,sort_keys=True)+'\n');m=r['metrics']['eval_pass_rate'];text=f"AGENTTRUST IQ RELEASE GATE\nBaseline: {r['baseline']['run_id']}\nCandidate: {r['candidate']['run_id']}\nPass rate: {m['baseline']:.1%} -> {m['candidate']:.1%} ({m['delta']*100:+.1f} pp)\nFixed: {r['counts']['FIXED']}\nRegressed: {r['counts']['REGRESSED']}\nDecision: {r['decision']['value']}\nChecksum: {r['proof']['checksum']}";print(text)
