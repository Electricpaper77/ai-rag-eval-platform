...
def run(name, cases):
    ...
    return {"run_id":name,"checksum":name+"-checksum","cases":cases,"metrics":{"eval_pass_rate":passed/total,"hallucination_rate":0.0,"citation_precision":1.0,"refusal_accuracy":1.0,"prompt_injection_defense_rate":1.0,"latency_p95_ms":10.0,"passed_cases":passed,"failed_cases":total-passed}}
def scenario(name):
    ...
    if name=="block": candidate["cases"][1]={"case_id":"injection","risk_category":"prompt_injection","pass":False,"failure_reasons":["prompt-injection refusal failed"]}
    return base,candidate
def main(argv=None):
    p=argparse.ArgumentParser();p.add_argument("--scenario",choices=("ship","block"),default="ship");p.add_argument("--output",type=Path,default=Path("artifacts/ci_release_proof.json"));a=p.parse_args(argv)
    try:
        result=compare(*scenario(a.scenario)); a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(result["proof"],indent=2,sort_keys=True)+"\n")
        m=result["metrics"]["eval_pass_rate"]; print(f"AGENTTRUST IQ RELEASE GATE\nBaseline: {result['baseline']['run_id']}\nCandidate: {result['candidate']['run_id']}\nPass rate: {m['baseline']:.1%} -> {m['candidate']:.1%} ({m['delta']:+.1%} pp)\nFixed: {result['counts']['FIXED']}\nRegressed: {result['counts']['REGRESSED']}\nDecision: {result['decision']['value']}\nEvidence: {a.output.as_posix()}\nChecksum: {result['proof']['checksum']}")
